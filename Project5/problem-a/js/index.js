...
let containsCool = motto.indexOf("cool");
if (containsCool == -1) {
    console.log(false);
} else {
    console.log(true);
...
let outdatedStr = "iSchool";
let index = motto.indexOf(outdatedStr);
let newStr = "Information School";
motto = motto.substring(0, index) + newStr + motto.substring(index + outdatedStr.length, motto.length);
console.log(motto);
...
let oldNewRatio = (updatedMottoLength / mottoLength * 100).toFixed(2) + "%";
console.log(oldNewRatio);
...
let median = numbers[medianIndex];
console.log(median);
...
// I got undefined because the hypotenuse key is not in the anonymous triangle object.
...
console.log(shapes);
// now the anonymous triangle object has a hypotenuse key with a value of 55.